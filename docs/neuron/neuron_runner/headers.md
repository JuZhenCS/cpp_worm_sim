...
```text
应用入口
  src/neuron/neuron_runner/runner.hpp
       |
用例层
...
把 `include/neuron/recording/recording.hpp` 中的数据写为trace CSV和diagnostics CSV。它不参与仿真，也不定义记录类型。
...
| 原设计 | 问题 | 当前设计 |
| --- | --- | --- |
| `types.hpp` | cell与trace混合 | `include/neuron/core/cell.hpp` + `include/neuron/recording/recording.hpp` |
| `recorder.hpp` | 数据结构与CSV输出混合 | `include/neuron/recording/recording.hpp` + `include/neuron/recording/csv_writer.hpp` |
| `neuron.hpp` | 文件名无法说明具体实现 | `include/neuron/core/multi_compartment_neuron.hpp` |
| `protocol.hpp` | 名称范围大于功能 | `include/neuron/protocol/clamp_protocol.hpp` |
...
新增protocol时先判断是否属于clamp。非clamp用例应建立独立模块，而不是继续扩大 `include/neuron/protocol/clamp_protocol.hpp`。
